...
_.extend(Mongo.prototype, {
    ...
    this.db.open(function (err, client) {
      if (err) {
        ...
        var finish = function (err) {
          self.client = client;
          self.lock = new mongo.Collection(client, options.collectionName);
  ...
  reserve: function(workerId, aggregateId, callback) {
    this.lock.save({ _id: workerId, aggregateId: aggregateId, date: new Date() }, { safe: true }, callback);
  },
  ...
  resolve: function(aggregateId, callback) {
    this.lock.remove({ aggregateId: aggregateId }, { safe: true }, callback);
  },

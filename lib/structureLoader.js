  ...
  return function (data) {
    ...
    if (!validation.valid) {
      if (validation.error.subErrors && validation.error.subErrors.length > 0) {
        ...
        throw new Error(validation.error.subErrors[0].dataPath + ' => ' + validation.error.subErrors[0].message);
      } else {
        throw new Error(validation.error.dataPath + ' => ' + validation.error.message);
      }
    }
  };
}
  ...
  obj.schemas.forEach(function (schemaItem) {
    ...
    obj.commands.forEach(function (cmdItem) {
      if (cmdItem.name === schemaItem.name &&
        ...
        var commands = ordered[schemaItem.context].getAggregates()[schemaItem.aggregate].getCommands();

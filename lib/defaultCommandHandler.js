  ...
  loadAggregate: function (aggregateId, callback) {
    ...
    this.eventStore.getFromSnapshot(query, function(err, snapshot, stream) {
      ...
      if (snapshot) {
        ...
        var ignoreSnapshot = self.aggregate.shouldIgnoreSnapshot(snapshot);

        if (ignoreSnapshot) {
          debug('skipping snapshot, and load whole event stream');

          startLoading = Date.now();
          self.eventStore.getEventStream(query, function(err, stream) {
            if (err) {
              return callback(err);
            }
            ...
            self.aggregate.loadFromHistory(aggregate, null, events, loadingTime);
            var isNewSnapShotNeeded = true;

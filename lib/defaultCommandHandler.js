...
//      },
      ...
      function (aggregate, stream, clb) {
        debug('check aggregate lock');
        self.checkAggregateLock(aggId, function (err) {
      ...
      self.resolveAggregateLock(aggId, function (errLock) {
        if (errLock) {
          debug(errLock);
        ...
        callback(null, eventsToDispatch || null, agg ? agg.toJSON() : null, meta);
      });
    });
  },
  ...
  handle: function (cmd, callback) {
    ...
    function _handle (aggId) {
      var isFirst = !self.getNextCommandInQueue(aggId);

      self.queueCommand(aggId, cmd, callback);

      if (!isFirst) {
        return;
      }

      (function handleNext (aggregateId) {
        var cmdEntry = self.getNextCommandInQueue(aggregateId);
        if (cmdEntry) {
          self.workflow(aggregateId, cmdEntry.command, function (err, evts, aggData, meta) {
            self.removeCommandFromQueue(aggregateId, cmdEntry.command);
            handleNext(aggregateId);

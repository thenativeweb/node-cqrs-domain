...
_.extend(DefaultCommandHandler.prototype, {
  ...
  loadAggregate: function (cmd, aggregateId, callback) {
    ...
    async.eachSeries(toLoad, function (loadInfo, callback) {
      ...
      self.eventStore.getFromSnapshot(loadInfo, function(err, snapshot, stream) {
        if (err) {
          return callback(err);
        }
        ...
        debug('needed ' + loadingTime + 'ms to load events' + addon + ' from the eventstore for: ', loadInfo);

        var isNewSnapShotNeeded = self.aggregate.loadFromHistory(aggregate, snapshot, events, loadingTime, stream);

        // check if new snapshot is needed........ per stream???

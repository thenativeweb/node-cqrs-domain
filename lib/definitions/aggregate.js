...
_.extend(Aggregate.prototype, {
  ...
  handle: function (aggregateModel, cmd, callback) {
    ...
    async.each(uncommittedEvents, function (evt, callback) {
      ...
      self.apply(uncommittedEvents, aggregateModel);

      // check business rules
      debug('check business rules');
      self.checkBusinessRules(aggregateModel, previousModel, uncommittedEvents, cmd, function (err) {
        if (!err) {
          if (self.dumpAggregate) {
            debug('dump aggregate');
            self.dumpAggregate(aggregateModel.toJSON(), {
              aggregateId: aggregateModel.id,
              aggregate: self.name,
              context: self.context ? self.context.name : undefined
            });
          }
          return callback(null);
        }
        ...
        // clean up...
        aggregateModel.set(previousModel.toJSON());
  ...
  loadFromHistory: function (aggregateModel, snapshot, events, loadingTime) {
    ...
    if (events && events.length > 0) {
      ...
      if (!isSnapshotNeeded) {
        isSnapshotNeeded = this.isSnapshotNeeded(loadingTime, events, aggregateModel);
      }
    }
  ...
  /**
   * Helper function Just for debug...
   * If defined it will pass the aggregate data, after business rules check.
   * @param   {Function}  fn The function to be injected.
   * @returns {Aggregate}    to be able to chain...
   */
  defineAggregateDumping: function (fn) {

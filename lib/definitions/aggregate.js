...
_.extend(Aggregate.prototype, {
  ...
  checkPreCondition: function (cmd, aggregateModel, callback) {
    ...
    if (!command.preCondition || !command.preCondition.payload) {
      if (this.defaultPreConditionPayload !== '') {
        return command.checkPreCondition(_.cloneDeep(dotty.get(cmd, this.defaultPreConditionPayload)), aggregateModel, callback);
      }
    }
    
    command.checkPreCondition(_.cloneDeep(cmd), aggregateModel, callback);
  },
  ...
  handle: function (aggregateModel, cmd, callback) {
    ...
    this.checkPreCondition(cmd, aggregateModel, function (err) {
      if (err) {
        return callback(err);
      }

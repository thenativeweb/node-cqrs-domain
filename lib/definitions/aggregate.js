...
function applyHelper (aggregate, aggregateModel, cmd) {
  return function (name, payload) {
    ...
    dotty.put(evt, aggregate.definitions.event.id, uuid().toString());
...
_.extend(Aggregate.prototype, {
  ...
  handle: function (aggregateModel, cmd, callback) {
    ...
    this.checkBusinessRules(aggregateModel.toJSON(), previousAttributes, aggregateModel.getUncommittedEvents(), cmd, function (err) {
      if (!err) {

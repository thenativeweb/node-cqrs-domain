 ...
 * @param {Object}   meta  Meta infos like: { name: 'name', version: 1, snapshotThreshold: 100 }
 * @constructor
 */
function Aggregate (meta) {  
  ...
  if (meta.snapshotThreshold && !_.isFunction(meta.snapshotThreshold)) {
    this.getSnapshotThreshold = function () { return meta.snapshotThreshold; };
  } else if (_.isFunction(meta.snapshotThreshold)) {
    this.getSnapshotThreshold = meta.snapshotThreshold;
  } else {
    this.getSnapshotThreshold = function () { return 100; };
  }
...
_.extend(Aggregate.prototype, {
  ...
  handle: function (aggregateModel, cmd, callback) {
    ...
    async.each(uncommittedEvents, function (evt, callback) {
      ...
      self.checkBusinessRules(aggregateModel.toJSON(), previousAttributes, uncommittedEvents, cmd, function (err) {
        if (!err) {
          return callback(null);
        }
  ...
  loadFromHistory: function (aggregateModel, snapshot, events) {
    ...
    if (events && events.length > 0) {
      ...
      if (!isSnapshotNeeded) {
        isSnapshotNeeded = this.isSnapshotNeeded(events, aggregateModel);
      }
    }
   ...
   * @param {AggregateModel} aggregateModel The aggregateModel object. [could be used for other algorithms]
   * @returns {boolean}
   */
  isSnapshotNeeded: function (events, aggregateModel) {
    return events.length >= this.getSnapshotThreshold();
  },

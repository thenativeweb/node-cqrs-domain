...
_.extend(Aggregate.prototype, {
   ...
   * @returns {boolean}
   */
  loadFromHistory: function (aggregateModel, snapshot, events, loadingTime, stream, streams) {
    var self = this;
    ...
    if (snapshot) {
      ...
      if (snapshot.context === this.context.name &&
          snapshot.aggregate === this.name &&
   ...
   * @returns {boolean}
   */
<<<<<<< a1b8817d3a67aa98eceed1bfa19403def9bb25ce
  isSnapshotNeeded: function (loadingTime, events) {
=======
  isSnapshotNeeded: function (loadingTime, events, aggregateModel, streams) {

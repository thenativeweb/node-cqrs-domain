...
_.extend(Command.prototype, {
  ...
  getLoadInfo: function (cmd) {
    ...
    if (_.isFunction(this.getOldStreamsInfo)) {
      var oldStreamsInfo = this.getOldStreamsInfo(cmd);
      if (!_.isArray(oldStreamsInfo)) {
        oldStreamsInfo = [oldStreamsInfo];
      }
      toLoad = oldStreamsInfo.concat(toLoad);
    }
  ...
  /**
   * Defines which (old) event streams should be loaded before handling this event.
   * @param {Function} fn Function containing the algorithm. Should return an array of infos.
   *                      `function(cmd){}`
   */
  defineOldEventStreamsToLoad: function (fn) {
    if (!_.isFunction(fn)) {
      throw new Error('Please pass in a function');
    }

    this.getOldStreamsInfo = fn;
    return this;
  }
